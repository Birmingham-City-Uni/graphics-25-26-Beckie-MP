		...
		for (int y = minY; y <= maxY; ++y) {
			...
			for (auto& light : lights) {
				...
				if (light->getType() != Light::Type::AMBIENT) {
					...
					Eigen::Vector3f incomingLightDir = light.get()->getDirection(worldP);
...
int main()
{
	...
	drawMesh(imageBuffer, zBuffer, bunnyMesh, Eigen::Vector3f(10.f, 0.5f, 20.8f), 
		Eigen::Vector3f::Ones()*1.0f, 90.f, camWorldPos,
