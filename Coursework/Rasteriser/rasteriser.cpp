	...
	const std::vector<uint8_t>& albedoTexture, int texWidth, int texHeight)
{
	...
	Eigen::Vector2f edge2 = v2(t.screen[2] - t.screen[0]);
	Eigen::Vector2f edge1 = v2(t.screen[1] - t.screen[0]);
...
int main()
{
	...
	lodepng::decode(bunnyTexture, bunnyTexWidth, bunnyTexHeight, "../Models/RPD_Texture.png");

	bunnyTransform = translationMatrix(Eigen::Vector3f(0.0f, -1.0f, 3.f)) * rotateYMatrix(M_PI) * scaleMatrix(0.05);
